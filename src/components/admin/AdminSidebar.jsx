...
const AdminSidebar = ({ activeSection, onSectionChange, isOpen, onClose }) => {
  ...
  return (
    <>
      {/* Overlay mobile: visibile solo quando la sidebar è aperta su schermi piccoli */}
      {isOpen && (
        <div
      ...
      <motion.aside
        initial={false}
        // Su desktop (lg) x è sempre 0, su mobile dipende da isOpen
        animate={{ x: window.innerWidth >= 1024 ? 0 : (isOpen ? 0 : '-100%') }}
        transition={{ type: 'spring', damping: 25, stiffness: 200 }}
        className="fixed top-0 left-0 h-full w-72 bg-slate-900/95 backdrop-blur-xl border-r border-slate-800 z-50 flex flex-col lg:static lg:translate-x-0"
      >
        <div className="p-6 border-b border-slate-800">
          ...
          <p className="text-[10px] text-gray-500 mt-1 truncate uppercase tracking-widest">{currentUser?.email}</p>
        </div>
